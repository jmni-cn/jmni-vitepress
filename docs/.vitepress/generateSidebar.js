...
export const cerateJson = () => {
    return new Promise(async (resolve, reject) => {
        ...
        async function transfileTree(fatherKey, key, cataloguePath) {
            ...
            if (fileStat.isDirectory()) {
            ...
            } else if (fileStat.isFile() && filePath.slice(filePath.length - 3, filePath.length) === '.md') {
                ...
                if (fatherKey.includes('/')) {
                    ...
                    childitem.filter(v => v.text === key2)[0] && childitem.filter(v => v.text === key2)[0].items.push({
                        text: filename,
